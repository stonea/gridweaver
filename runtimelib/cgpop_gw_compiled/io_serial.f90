...
module io_serial
    ...
    subroutine read_tiles_dbl(fname,vname,var)
        ...
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid: '//vname,85)
        ierr = nf90_get_var(fh,varid, var)
        if(ierr /= nf90_noerr) &
        call check_netcdf(fh,ierr,'nf90_get_var: '//vname,88)
    ...
    subroutine read_tiles_int(fname,vname,var)
        ...
        ierr = nf90_inq_varid(fh,TRIM(vname),varid)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid: '//vname,130)

        ierr = nf90_get_var(fh,varid, var)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_get_var: '//vname,134)
    ...
    subroutine write_tiles_int(fname,vname,var)
        ...
        if(ierr /= nf90_noerr) then 
            ...
            if(ierr /= nf90_noerr) &
                call check_netcdf(fh,ierr,'nf90_def_dim',169)
        endif
        ...
        if(ierr /= nf90_noerr) then 
            ...
            if(ierr /= nf90_noerr) &
                call check_netcdf(fh,ierr,'nf90_def_dim',176)
        endif
        ...
        if(ierr /= nf90_noerr) then 
            ...
            if(ierr /= nf90_noerr) &
                call check_netcdf(fh,ierr,'nf90_def_dim',183)
        endif
        ...
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_def_var',190)
        ...
        ierr = nf90_enddef(fh)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_enddef',197)
        ...
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_put_var',204)
        ierr = nf90_close(fh)
    ...
    subroutine write_tiles_dbl(fname,vname,var)
        ...
        if(ierr /= nf90_noerr) then
            ierr = nf90_def_dim(fh,name='nblocks',len=nblocks,dimid=dim_nb)
            if(ierr /= nf90_noerr) &
                call check_netcdf(fh,ierr,'nf90_def_dim',238)
        endif
        ...
        if(ierr /= nf90_noerr) then
            ierr = nf90_def_dim(fh,name='nx_block',len=nx_block,dimid=dim_nx)
            if(ierr /= nf90_noerr) &
                call check_netcdf(fh,ierr,'nf90_def_dim',245)
        endif
        ...
        if(ierr /= nf90_noerr) then
            ierr = nf90_def_dim(fh,name='ny_block',len=ny_block,dimid=dim_ny)
            if(ierr /= nf90_noerr) &
                call check_netcdf(fh,ierr,'nf90_def_dim',252)
        endif
        ...
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_def_var',259)
        ...
        ierr = nf90_enddef(fh)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_enddef',266)
        ...
        ierr = nf90_put_var(fh,ncid,var)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_put_var',273)
        ierr = nf90_close(fh)
    ...
    subroutine write_AppMD(fname,blocks,reorder)
        ...
        ierr = nf90_def_dim(fh,name='nblocks',len=nblocks,dimid=dim_nb)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_def_dim',316)
        ierr = nf90_def_dim(fh,name='nx_block',len=nx_block,dimid=dim_nx)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_def_dim',319)
        ierr = nf90_def_dim(fh,name='ny_block',len=ny_block,dimid=dim_ny)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_def_dim',322)
        ...
        ierr = nf90_enddef(fh)
        if(ierr /= nf90_noerr)  &
            call check_netcdf(fh,ierr,'nf90_enddef',394)
        ...
        if(ierr /= nf90_noerr)  &
            call check_netcdf(fh,ierr,'nf90_put_var',401)
        ierr = nf90_put_var(fh,ncid_ib,blocks(:)%ib)
        if(ierr /= nf90_noerr)  &
            call check_netcdf(fh,ierr,'nf90_put_var',404)
        ierr = nf90_put_var(fh,ncid_ie,blocks(:)%ie)
        if(ierr /= nf90_noerr)  &
            call check_netcdf(fh,ierr,'nf90_put_var',407)
        ierr = nf90_put_var(fh,ncid_jb,blocks(:)%jb)
        if(ierr /= nf90_noerr)  &
            call check_netcdf(fh,ierr,'nf90_put_var',410)
        ierr = nf90_put_var(fh,ncid_je,blocks(:)%je)
        if(ierr /= nf90_noerr)  &
            call check_netcdf(fh,ierr,'nf90_put_var',413)
        ierr = nf90_put_var(fh,ncid_iblock,blocks(:)%iblock)
        if(ierr /= nf90_noerr)  &
            call check_netcdf(fh,ierr,'nf90_put_var',416)
        ierr = nf90_put_var(fh,ncid_jblock,blocks(:)%jblock)
        if(ierr /= nf90_noerr)  &
            call check_netcdf(fh,ierr,'nf90_put_var',419)
        ierr = nf90_put_var(fh,ncid_npoints,blocks(:)%npoints)
        if(ierr /= nf90_noerr)  &
            call check_netcdf(fh,ierr,'nf90_put_var',422)
        ...
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_put_var',431)
        ierr = nf90_put_var(fh,ncid_jglob,jglob)        
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_put_var',434)
        ierr = nf90_put_var(fh,ncid_reorder,reorder)        
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_put_var',437)
        ierr = nf90_close(fh)
    ...
    subroutine read_AppMD(fname,blocks,reorder)
        ...
        ierr = nf90_inq_dimid(fh,'nblocks',dimid=dim_nb)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_dimid',468)

        ierr = nf90_inq_dimid(fh,'nx_block',dimid=dim_nx)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_dimid',472)

        ierr = nf90_inq_dimid(fh,'ny_block',dimid=dim_ny)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_dimid',476)

        ierr = nf90_inquire_dimension(fh,dim_nb,len=nblocks)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inquire_dimension',480)

        ierr = nf90_inquire_dimension(fh,dim_nx,len=nx_block)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inquire_dimension',484)

        ierr = nf90_inquire_dimension(fh,dim_ny,len=ny_block)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inquire_dimension',488)
        ...
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid',497)

        ierr = nf90_get_var(fh,varid, blocks(:)%block_id)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_get_var',501)

        ierr = nf90_inq_varid(fh,'ib',varid)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid',505)

        ierr = nf90_get_var(fh,varid, blocks(:)%ib)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_get_var',509)

        ierr = nf90_inq_varid(fh,'ie',varid)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid',513)

        ierr = nf90_get_var(fh,varid, blocks(:)%ie)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_get_var',517)

        ierr = nf90_inq_varid(fh,'jb',varid)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid',521)

        ierr = nf90_get_var(fh,varid, blocks(:)%jb)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_get_var',525)

        ierr = nf90_inq_varid(fh,'je',varid)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid',529)

        ierr = nf90_get_var(fh,varid, blocks(:)%je)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_get_var',533)

        ierr = nf90_inq_varid(fh,'iblock',varid)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid',537)

        ierr = nf90_get_var(fh,varid, blocks(:)%iblock)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_get_var',541)

        ierr = nf90_inq_varid(fh,'jblock',varid)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid',545)

        ierr = nf90_get_var(fh,varid, blocks(:)%jblock)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_get_var',549)

        ierr = nf90_inq_varid(fh,'npoints',varid)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid',553)

        ierr = nf90_get_var(fh,varid, blocks(:)%npoints)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_get_var',557)
        ...
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid',564)
        ierr = nf90_get_var(fh,varid, iglob)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_get_var',567)
        ierr = nf90_inq_varid(fh,'j_glob',varid)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid',570)
        ierr = nf90_get_var(fh,varid, jglob)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_get_var',573)
        ...
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_inq_varid',587)
        
        ierr = nf90_get_var(fh,varid, reorder)
        if(ierr /= nf90_noerr) &
            call check_netcdf(fh,ierr,'nf90_get_var',591)
    ...
    function sio_open(fname,mode,fh)
        ...
        if(ierr /= nf90_noerr) then
            call check_netcdf(fh,ierr,'nf90_open(): '//fname,630)
